...
type pageArg = { q_id: number, page: number }
export const handleLoadAnswers = createAsyncThunk("a/all",
	({ q_id, page }: pageArg) => aApi.fetchQuestionAnswers(q_id, page), {
	condition: (args, { getState }) =>
		// q_id
		!getState().answers.fetchedPages.includes(args.page),
	});
...
export const handleAddAnswer = createAsyncThunk("a/add", ({ content, q_id }: ansArg) => aApi.store(content, q_id));
