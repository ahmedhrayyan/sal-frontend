...
export const handleLoadAnswers = createAsyncThunk(
	"a/all",
...
export const handleDeleteAnswer = createAsyncThunk("a/delete", (a: Answer) =>
	aApi.remove(a.id)
);
...
const slice = createSlice({
	...
	extraReducers: (builder) => {
		builder
			...
			.addMatcher(
				isFulfilled(handleShowAnswer, handleAddAnswer, handleUpdateAnswer),
				(state, { payload }) => {
					state.status = "succeeded";
					aAdapter.upsertMany(state, payload.entities.answers);
			...
			.addMatcher(isPending(handleLoadAnswers, handleShowAnswer), (state) => {
				state.status = "pending";
			})
			.addMatcher(isPending(handleAddAnswer, handleUpdateAnswer), (state) => {
...
export const selectAStatus = (state: RootState) =>
	state.answers.status as LoadingStatus;
